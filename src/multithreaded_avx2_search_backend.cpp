...
bool MultithreadedAVX2SearchBackend::is_supported() noexcept {
    return AVX2SearchBackend::is_supported();
}

MultithreadedAVX2SearchBackend::MultithreadedAVX2SearchBackend(
    std::size_t dimension, std::size_t thread_count)
    : dimension_(dimension), thread_count_(thread_count) {
    // Baseline-architecture guard runs before any worker can enter the SIMD kernel.
    if (!is_supported()) {
        throw std::runtime_error("Multithreaded AVX2 backend unsupported: requires AVX2, FMA and OS YMM support");
    }
...
std::size_t MultithreadedAVX2SearchBackend::dimension() const noexcept { return dimension_; }
std::size_t MultithreadedAVX2SearchBackend::size() const noexcept { return entries_.size(); }
std::size_t MultithreadedAVX2SearchBackend::thread_count() const noexcept { return thread_count_; }

void MultithreadedAVX2SearchBackend::add(std::string id, std::vector<float> values) {
...
const std::vector<float>& MultithreadedAVX2SearchBackend::get(std::string_view id) const {
...
std::vector<SearchResult> MultithreadedAVX2SearchBackend::search(
    ...
    // Mirror the established scalar partition/merge strategy; existing backends stay unchanged.
    ...
    auto scan = [&](std::size_t worker) {
        try {
            ...
            for (auto i = begin; i < end; ++i) {
                results.push_back({entries_[i].id, detail::avx2_cosine_similarity(query, entries_[i].values)});
            }
