...
ScalarSearchBackend::ScalarSearchBackend(const std::size_t dimension)
...
std::size_t ScalarSearchBackend::dimension() const noexcept {
    return dimension_;
}

std::size_t ScalarSearchBackend::size() const noexcept {
    return entries_.size();
}

void ScalarSearchBackend::add(std::string id, std::vector<float> values) {
...
const std::vector<float>& ScalarSearchBackend::get(const std::string_view id) const {
...
std::vector<SearchResult> ScalarSearchBackend::search(
    ...
    for (const Entry& entry : entries_) {
        results.push_back(SearchResult{
            entry.id,
            cosine_similarity(query, entry.values),
        });
    }
